...
import { createLotTemp, deleteLotTemp, generateBulletinTemp, getLotsTemp, updateLotTemp } from '../services/lotstempservice';
...
function LotsTemporaire() {
    ...
    const {isLoading } = useQuery(qk, () => getLotsTemp(),{
      onSuccess: (_) => {
    ...
    const {mutate: create,isLoading:isLoadingc} = useMutation((data) => createLotTemp(data), {
        onSuccess: (_) => {
            notifications.show({
                title: 'CREATION',
    ...
    const {mutate: genBulletin,isLoading:isLoadingG} = useMutation((id) => generateBulletinTemp(id), {
      onSuccess: (_) => {
    ...
    const {mutate: update,isLoading: isLoadingu} = useMutation((data) => updateLotTemp(data._id, data.data), {
        onSuccess: (_) => {
            notifications.show({
                title: 'MIS A JOUR',
    ...
    const {mutate: supprimer,isLoading:isLoadingde} = useMutation((id) => deleteLotTemp(id), {
      onSuccess: (_) => {
          notifications.show({
              title: 'SUPPRESSION',
    ...
    const handleViewLot = (row) => {
      ...
      setPdf({uri:`${import.meta.env.VITE_BACKURL}/uploads/bulletins/temporaires/${fileName}`,fileName});
  }
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between align-items-center">
                <h5 className="m-0">LISTE DES LOTS DES TEMPORAIRES</h5>
