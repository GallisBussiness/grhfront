...
import { FaEye,FaFolder,FaPrint,FaSearch, FaXing } from 'react-icons/fa'
import { useNavigate } from 'react-router-dom'
import { IconButton, toaster } from 'evergreen-ui'
import { ActionIcon, Badge, Box, Button, Group, LoadingOverlay, Stack, Switch, TextInput } from '@mantine/core'
...
function Cdds() {
  ...
  const [selectedGenre, setSelectedGenre] = useState('all');
    ...
    const filtered = (Employes = []) => {
        ...
        return sortStatus.direction === 'desc' ? emp.reverse() : emp?.filter(({ prenom,nom,poste,date_de_recrutement}) => {
          if (
            debouncedQuery !== '' &&
      ...
      useEffect(() => {
          ...
          setRecords(filtered(Cdds).slice(from, to) ?? []);
        }, [page,Cdds,debouncedQuery,posteQuery,recrutementSearchRange,sortStatus]);
    ...
    <Can I='manage' a={role}>
      ...
      columns={[ 
      {
        ...
        footer: (
          <Group gap="xs">
            ...
            <div>{filtered(Cdds)?.length} employees / {Cdds?.length}</div>
          </Group>
        ),
      },
      {
        ...
        filter: ({ close }) => (
          <Stack>
            ...
            <Button
              ...
              variant="light"
              onClick={() => {
                setRecrutementSearchRange(undefined);
                close();
              }}
            >
              Clear
      ...
      },
      {
        ...
        footer: `Moy: ${Math.round(
          Cdds?.map((record) => dayjs().diff(record.date_de_naissance, 'years')).reduce((a, b) => a + b, 0) / Cdds?.length
        )} ans`,
      },
      {
        accessor: 'Actif',
      ...
      fetching={isLoading}
      totalRecords={filtered(Cdds)?.length}
