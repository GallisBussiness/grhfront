...
function Fonctions() {
    ...
    const qk = ['get_Fonctions']

    const {data: Fonctions, isLoading } = useQuery(qk, () => getFonctions());

    const {mutate: create,isLoading:isLoadingc} = useMutation((data) => createFonction(data), {
        onSuccess: (_) => {
            notifications.show({
                title: 'CREATION',
    ...
    const {mutate: update,isLoading: isLoadingu} = useMutation((data) => updateFonction(data._id, data.data), {
        onSuccess: (_) => {
            notifications.show({
                title: 'MIS A JOUR',
    ...
    const leftToolbarTemplate = () => {
        return (
            <>
                <Button variant="gradient"  leftSection={<AiOutlinePlus className="h-6 w-6 text-white"/>} onClick={() => handleCreateFonction()}>Nouveau</Button>
            </>
        )
    }
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between align-items-center">
                <h5 className="m-0">LISTE DES FONCTIONS</h5>
  ...
  <LoadingOverlay visible={isLoadingc || isLoading || isLoadingu || isLoadingde} overlayProps={{ radius: 'sm', blur: 2 }} loaderProps={{ color: 'blue', type: 'bars' }} />
    ...
    <div className="datatable-doc">
         ...
         <Toolbar className="mb-4" left={leftToolbarTemplate}></Toolbar>
             <DataTable value={Fonctions} paginator className="p-datatable-customers" header={header} rows={10}
   ...
   <Modal opened={opened} onClose={close} title={curFonction ? "UPDATE FONCTION" : "CREATION FONCTION"} centered>
          ...
          <div className="flex items-center justify-center">
            <div>
              <Button type="submit"  variant="gradient">
               {curFonction ? "MODIFIER LA FONCTION" : "CREER LA FONCTION"}
              </Button>
            </div>
          </div>
        </form>
