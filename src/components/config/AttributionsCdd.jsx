...
import { createAttributionglobaleCdd, getAttributionglobalesCdd, removeAttributionGlobaleCdd, updateAttributionglobaleCdd } from '../../services/attributionGlobalCdd';
...
function AttributionsCdd() {
    ...
    const qk = ['get_AttributionsCdd']

    const {data: Attributionglobales, isLoading } = useQuery(qk, () => getAttributionglobalesCdd());
    ...
    const {mutate: create,isLoading:isLoadingc} = useMutation((data) => createAttributionglobaleCdd(data), {
        onSuccess: (_) => {
            notifications.show({
                title: 'CREATION',
    ...
    const {mutate: update,isLoading: isLoadingu} = useMutation((data) => updateAttributionglobaleCdd(data._id, data.data), {
        onSuccess: (_) => {
            notifications.show({
                title: 'MIS A JOUR',
    ...
    const {mutate: supprimer,isLoading:isLoadingde} = useMutation((id) => removeAttributionGlobaleCdd(id), {
      onSuccess: (_) => {
          notifications.show({
              title: 'SUPPRESSION',
    ...
    const handleDeleteAttributionGlobale = (event,row) => {
      confirmPopup({
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between align-items-center">
                <h5 className="m-0">LISTE DES ATTRIBUTIONS CDD</h5>
    ...
    const actionBodyTemplate = (rowData) => {
        ...
        <IconButton onClick={(event) => handleDeleteAttributionGlobale(event,rowData)} icon={<FaTrash className="text-red-500"/>} />
  ...
  return (
    ...
    <LoadingOverlay visible={isLoadingc || isLoading || isLoadingu || isLoadingR || isLoadingde} overlayProps={{ radius: 'sm', blur: 2 }} loaderProps={{ color: 'blue', type: 'bars' }} />
     ...
     <Modal opened={opened} onClose={close} title={curAttributionglobale ? "UPDATE ATTRIBUTION" : "CREATION ATTRIBUTION"} centered>
            ...
            <div className="flex items-center justify-center">
              <div>
                <Button type="submit"  variant="gradient">
                 {curAttributionglobale ? "MODIFIER LA ATTRIBUTION" : "CREER LA ATTRIBUTION"}
                </Button>
              </div>
            </div>
          </form>
